...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs/baseline_training_target5.toml",
    )
    ...
    report = json.loads((ROOT / dataset_cfg["report"]).read_text(encoding="utf-8"))
    if report["status"] not in {
        "TARGET5_DATASET_READY",
        "ROBOT_DOMAIN_ADAPTATION_READY",
        "TARGET5_AUGMENTED_DATASET_READY",
    } or report["not_target_present"]:
        raise RuntimeError("target-five dataset contract is not ready")
    ...
    if sha256_file(manifest) != report["manifest_sha256"]:
        raise RuntimeError("target-five manifest checksum mismatch")
    layout = {
        split: {
            label: len(list((dataset_root / split / directory).glob("*")))
            for label, directory in zip(
                dataset_cfg["labels"], dataset_cfg["class_directories"]
            )
        }
        for split in report["final_counts"]
    }
    if layout != report["final_counts"]:
    ...
    output = {
        "schema_version": 1,
        "status": "TARGET5_TRAINING_COMPLETE",
        "model_id": model_cfg["id"],
        ...
        "native_names": YOLO(str(best), task="classify").names,
    }
    (actual_run / "target5_training_report.json").write_text(
