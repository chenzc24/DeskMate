"""Interpolate general and robot-adapted target-five checkpoints."""
...
from deskmate_baseline.target_inference import target_index_mapping
...
def main() -> int:
    config_path = ROOT / "configs/baseline_target5_robot_soups.toml"
    with config_path.open("rb") as handle:
        config = tomllib.load(handle)
    base_path = ROOT / config["sources"]["general"]
    adapted_path = ROOT / config["sources"]["robot_adapted"]
    ...
    report = {
        "schema_version": 1,
        "status": "TARGET5_ROBOT_WEIGHT_SOUPS_READY",
        "sources": {
            "general": {"path": str(base_path), "sha256": sha256_file(base_path)},
            "robot_adapted": {
    ...
    for name, alpha_value in config["alphas"].items():
        ...
        if not 0.0 <= alpha <= 1.0:
            raise ValueError(f"invalid robot coefficient: {name}={alpha}")
        ...
        destination = output / f"target5_robot_alpha_{alpha:.1f}.pt"
        torch.save(checkpoint, destination)
        target_index_mapping(YOLO(str(destination), task="classify").names)
        report["candidates"][name] = {
            "robot_coefficient": alpha,
            "general_coefficient": 1.0 - alpha,
