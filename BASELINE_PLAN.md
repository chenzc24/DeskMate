# DeskMate：本地 4070 Baseline 完整计划

> 版本：v0.2（Multi-Expert Perception）
> 日期：2026-07-14
...
项目定位为 **桌面伴侣机器人 DeskMate**。Baseline 只实现一条完整主线：
...
Baseline 的成功标准不是“功能最多”，而是 **同一套代码能够连续稳定完成完整 Demo，且任何感知或通信故障都会安全停车**。多专家模型只改造感知层，不改变 `WorldState → Semantic Event → FSM → Safety Gate` 的主链路。
...
- 机器人最好提供前方距离或碰撞状态；如果没有，Baseline 取消自动前进，只保留原地搜索和指示目标。
...
- 使用一个模块化 Python 进程，不在 Baseline 阶段拆微服务，也不引入 ROS。
...
|---|---|
| 语言 | Python 3.11 |
| 全局物体专家 | PyTorch + Ultralytics `yolo26n.pt`，RTX 4070 |
...
| Baseline UI | OpenCV 单窗口叠加层；Web UI 仅作为 P1 |
...
|---|---|---|---|---|---|---|---|
| `M01` | P0 必需 | Ultralytics YOLO26n | [Hugging Face: `Ultralytics/YOLO26`](https://huggingface.co/Ultralytics/YOLO26)，运行权重 `yolo26n.pt` | RTX 4070 / PyTorch | 人、手机、瓶子、杯子、书本、笔记本电脑检测 | **是，唯一强制微调模型** | HF 标注 AGPL-3.0 |
...
| `M01` YOLO26n | 始终 | 8–12 Hz | RTX 4070 | 300 ms |
...
Baseline 不训练大型行为视频模型，而是将 `M01/M02/M03` 的检测、面部/眼部和手部输出组合成结构化语义。P0 使用可解释的证据融合；只有在积累足够的专家特征序列后才训练 `M05`。
...
如果书本和笔记本电脑均无法可靠检测，Baseline 将 `working` 定义为“人员在场且未使用手机”，并在海报中如实说明。
...
## 17. Baseline Definition of Done

只有同时满足以下条件才算 Baseline 完成：
